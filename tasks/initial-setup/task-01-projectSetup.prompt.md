...
- [x] Dependências de produção instaladas (`react-router-dom`, `axios`, `styled-components`, `react-icons`)
- [x] Dependências de desenvolvimento instaladas (`@stryker-mutator/core`, `@stryker-mutator/jest-runner`, `@stryker-mutator/html-reporter`, `@playwright/test`)
- [x] Mover `@testing-library/*` de `dependencies` para `devDependencies`
- [x] Scripts adicionados ao `package.json` (`lint`, `test:coverage`, `test:mutation`, `test:e2e`, `test:e2e:ui`)
- [x] Configuração de `jest.coverageThreshold` no `package.json` (mínimo 80%)
- [x] Arquivo `stryker.config.mjs` criado e configurado (com `htmlReporter.baseDir`)
- [x] Arquivo `playwright.config.js` criado e configurado (com `webServer` para subir a app)
- [x] Arquivo `.env` com `REACT_APP_API_URL=http://localhost:3000`
- [x] Arquivo `.env.example` versionado como referência
- [x] Estrutura de pastas DDD criada (pastas com arquivos reais serão versionadas; demais serão criadas nas tasks seguintes)
- [x] `GlobalStyles.js` e `theme.js` base criados (com conteúdo mínimo funcional)
- [x] `httpClient.js` configurado com instância Axios
- [x] `.gitignore` atualizado (incluir `.idea/`, reports, node_modules, etc.)
- [x] GitHub Actions: workflow de CI criado (`.github/workflows/ci.yml`)
- [x] GitHub Actions: workflow de mutation testing criado (`.github/workflows/mutation.yml`)
- [x] GitHub Actions: workflow de E2E criado (`.github/workflows/e2e.yml`)
...
- [x] Rodar `npm start` sem erros
- [x] Rodar `npm test` sem erros (testes existentes passando)
- [x] Rodar `npm run lint` sem erros
- [x] Estrutura de pastas DDD criada corretamente (verificar com `find src/shared`)
- [x] Variável de ambiente `REACT_APP_API_URL` configurada
- [x] `@testing-library/*` em `devDependencies` (não em `dependencies`)
- [x] Workflows do GitHub Actions criados (validação real ocorrerá após push na Task 06)
